...
Lambda.prototype._npmInstall = function (program, codeDirectory, callback) {
  var command = program.dockerContainer ?
    'docker run --rm -v ' + codeDirectory + ':/var/task ' + program.dockerContainer + ' npm -s install --production' :
...
Lambda.prototype._buildAndArchive = function (program, archive_callback) {
  ...
  if (arch !== 'linux.x64' && !program.dockerContainer) {
