...
Lambda.prototype.setup = function (program) {
  ...
  this._createSampleFile(program.configFile, 'deploy.env');
  this._createSampleFile(program.contextFile, 'context.json');
  this._createSampleFile(program.eventSourceFile, 'event_sources.json');
...
Lambda.prototype._fileCopy = function (program, src, dest, excludeNodeModules, callback) {
  ...
  const excludes = (function () {
    ...
    .concat(program.configFile ? [program.configFile] : [])
    .concat(program.eventSourceFile ? [program.eventSourceFile] : []);
  })();
